...
export const unwrapOption = <T>(opt: Option<T>, msg?: string): T => {
  if (isNone(opt)) throw new Error(msg ?? 'called `unwrapOption` on a `None` value');
  return opt.value;
};
...
export const unwrapResult = <T, E>(res: Result<T, E>, msg?: string): T => {
  if (isErr(res)) throw new Error(msg ?? 'called `unwrapResult` on an `Err` value');
  return res.value;
};
...
export const unwrapErr = <T, E>(res: Result<T, E>, msg?: string): E => {
  if (isOk(res)) throw new Error(msg ?? 'called `unwrapErr` on an `Ok` value');
  return res.error;
};
...
export const pipeOption = <T>(opt: Option<T>) => ({
  isSome: () => isSome(opt),
  isNone: () => isNone(opt),
...
export const pipeResult = <T, E>(res: Result<T, E>) => ({
  isOk: () => isOk(res),
  isErr: () => isErr(res),
